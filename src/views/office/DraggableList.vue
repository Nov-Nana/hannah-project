...
const { StarOutlined, DragOutlined } = icon.antd
const tableRef = ref()
const tableData = [
    {
        "id": 1,
...
const tableColumns = [
    ...
    },
    {
        ...
        title: '重要性',
        key: 'importance',
        render: (row: { importance: number; }) => {
            let starAll = new Array()
            for (let i = 0; i < row.importance; i++) {
                let star = h(NIcon, null, { default: () => h(StarOutlined, undefined, undefined) })
                starAll.push(star)
            }
            return h('span', null, starAll)
        },
        ellipsis: {
            tooltip: true
        }
    },
    {
        ...
        render: (row: { status: string; }) => {
            return h(NButton, {
                ...
                size: 'small',
                secondary: true,
                type: row.status === 'draft' ? 'tertiary' : row.status === 'success' ? 'info' : 'primary'
            }, { default: () => row.status })
        },
        ellipsis: {
            tooltip: true
    ...
    },
    {
        ...
        key: 'id',
        render: () => {
            return h(NIcon, {size: 20}, { default: () => h(DragOutlined, undefined, undefined) })
        },
        ellipsis: {
            tooltip: true
        }
...
const initializeSortable = () => {
    nextTick(() => {
        if (tableRef.value) {
            ...
            Sortable.create(el, {
                onEnd: (evt: any) => {
                    // 截下要拖拽的位置， [1,2,3,4,5].splice(4,1)[0] => 5
                    const cutRow = tableData.splice(evt.oldIndex, 1)[0];
                    tableData.splice(evt.newIndex, 0, cutRow);
                }
            })
        }
    })
...
<template>
    <div class="drag-table">
        <n-data-table ref="tableRef" :data="tableData" :columns="tableColumns" :row-props="rowProps">
        </n-data-table>
    </div>
</template>
